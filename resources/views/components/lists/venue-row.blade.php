...
<li class="flex justify-between gap-x-6 py-4">
    <div class="flex min-w-0 gap-x-4">
        <div class="min-w-0 flex-auto">
            <p class="text-sm font-semibold leading-6 text-secondary-900">{{ $venue->name }}</p>
    ...
    <div class="flex shrink-0 items-center gap-x-6" x-data="{openOptions: false}">
        <div class="hidden sm:flex sm:flex-col sm:items-end">
            <p class="text-sm leading-6 text-secondary-900">{{ $venue->country }}</p>
            <p class="mt-1 text-xs leading-5 text-secondary-500">{{ $venue->zip_code }}</p>
        </div>
            ...
            <div x-transition:enter="transition ease-out duration-100"
                ...
                <form action="{{ route('admin.venues.destroy', $venue) }}" method="post"
                    ...
                    <button role="menuitem" tabindex="-1" id="options-menu-0-item-1">
                        Supprimer<span class="sr-only">, {{ $venue->title }}</span>
                    </button>
                </form>
            </div>
        </div>
