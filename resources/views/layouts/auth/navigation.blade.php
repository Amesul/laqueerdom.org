...
<nav class="bg-secondary border-b border-primary/10 shadow-lg relative z-40" x-data="{openMobileMenu: false}">
                ...
                <div class="hidden lg:block">
                    <div class="ml-10 flex items-baseline space-x-1 lg:space-x-4">
                        ...
                        @if(str_contains($_SERVER['REQUEST_URI'], 'profile'))
                            <x-dashboard.account-navigation/>
                        @endif
                        @if(str_contains($_SERVER['HTTP_HOST'], 'artist'))
                            <x-dashboard.artist-navigation/>
                        @endif
                        @if(str_contains($_SERVER['HTTP_HOST'], 'admin'))
                            <x-dashboard.admin-navigation/>
                        @endif
                    </div>
                </div>
            </div>
                ...
                <div class="ml-4 flex items-center md:ml-6">
                    <div x-data="{ openNotifications: false }" class="relative">
                        ...
                        <div x-show="openNotifications" @click.outside="openNotifications = false"
                             x-transition
                             class="bg-white absolute top-10 right-0 rounded-md p-4 shadow-lg w-96">
                            Prochainement...
                        </div>
                    </div>
    ...
    <div x-show="openMobileMenu" @click.outside="openMobileMenu = false" class="lg:hidden" id="mobile-menu">
